...
##	print "$forwardLimit|$backwardLimit\n";
	...
	#for (my $i = 0; $i < scalar (@PolyploidWindow); $i++){
		...
		for (my $j = 0; $j < scalar (@OutgroupWindow); $j++){ # Foreach gene in the outgroup windows
			...
			if (exists $superHash{$anchor[0]}{$OutgroupWindow[$j]}{$anchor[2]}){ # If there are orthologs on current polyploid chromosome for this anchor
...
#					print " $_";
					...
					if ($anchor[0] ne $anchor[2]){ # outgroup and polyploid chromosomes are different
					
						if ($_ >= $leftPolyploidWd && $_ <= $rightPolyploidWd){ # Check if the gene lie within the polyploid window. Because the booundary is the part of this block i have the condition as >= and not >. Earlier in older version it was > which was wrong!
#							print "*";
							push @block, [$anchor[0], $OutgroupWindow[$j], $anchor[2], $_]; # Push them to block array
							$blockSize++;
						}
					}
					else { # If outgroup and polyploid organisms are same
						
						if ($OutgroupWindow[$j] < $_){ # and outgroup location is smaller than polyploid, then 
						
							if ($_ >= $leftPolyploidWd && $_ <= $rightPolyploidWd && $rightOutgroupWd < $leftPolyploidWd){ # Check if the gene lie within the polyploid window. And since the gene is on same chromosome, the right Og window is greater than left Pp wd - so that the two windows do not overlap
								push @block, [$anchor[0], $OutgroupWindow[$j], $anchor[2], $_]; # Push them to block array
								$blockSize++;
							}
						}
						if ($OutgroupWindow[$j] > $_){ # Else if outgroup location is greater than polyploid, then 
						
							if ($_ >= $leftPolyploidWd && $_ <= $rightPolyploidWd && $leftOutgroupWd > $rightPolyploidWd){ # Check if the gene lie within the polyploid window. And since the gene is on same chromosome, the left Og window is greater than right Pp wd - so that the two windows do not overlap
								push @block, [$anchor[0], $OutgroupWindow[$j], $anchor[2], $_]; # Push them to block array
								$blockSize++;
							}
						}						
