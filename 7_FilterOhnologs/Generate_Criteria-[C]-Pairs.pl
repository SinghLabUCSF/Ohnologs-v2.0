...
my $og = 10000; # A number that will give warning
my $self = 10000;
...
#	if ($organism eq 'hsapiens'){ # test for 1 org
	...
	# Open outfile	
	my $outfile = "$organism\/".$organism.'_Criteria-[C]-Pairs'.$wgd.'.txt';
	...
	while (<FH>){
	   ...
	   if ($line[0] !~/^ENS.+/){print OUT $_; next;} # Print and skip to next for header
		...
		# Criteria [C]
		if ($line[$og] ne '' && $line[$self] ne '' && $line[$og] < 0.05 && $line[$self] < 0.3){
			print OUT "$_";
		}
	}
#	} # end test for 1 org
