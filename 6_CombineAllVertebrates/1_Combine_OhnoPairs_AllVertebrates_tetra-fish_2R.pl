...
my $wgdtype = '_2R'; # WGD type **************** # use '_2R' 
#my $paralog_path = '../2_Paralogs/No-Sarco-Neo/4_filter_multi_copy_paralogs_No-Sarco-Neo';
my $paralog_path = '../2_Paralogs/4_filter_multi_copy_paralogs';
my $ohnofilepath = '../5_Run_synteny/5_CombineSelfOutgroup';
my $totalOrganisms = 26; # This should be total organisms to be combined - 1 (as the starting organism will be the first one)
...
if (($wgdtype eq '_3R') && ($organism ~~ @tetrapods)){die "3R is not allowed for tetrapods\n";}
else {print "  WGD type: $wgdtype\n";}

# --------------------------------------------------------------------------------------------------------------------

# Decide the input file name for tetrapods or fish
my $combinedOhnoFile;
if ($organism ~~ @fish){$combinedOhnoFile = "$organism"."Ohno_Self+Outgp$wgdtype\_withDetails.txt";}
else {$combinedOhnoFile = "$organism"."Ohno_Self+Outgp_withDetails.txt";}

# --------------------------------------------------------------------------------------------------------------------
# *********************** Decide the organism names and order in which to add other vertebrates
my @organisms; # this array will have all the organisms that I need to add to the ohnolog dataset
if ($organism ~~ @fish){ # If organism is fish for 2R add all fish and tetraods --> put the fish first. For 3R just add fish. THIS IS ALSO THE ORDER IN WHICH ORGANISMS WILL BE ADDED.
	
	if ($wgdtype eq '_2R'){@organisms = ('tnigroviridis', 'gaculeatus', 'olatipes', 'drerio', 'hsapiens', 'mmusculus', 'rnorvegicus', 'ptroglodytes', 'ggorilla', 'pabelii', 'panubis', 'csabaeus', 'mmulatta', 'cjacchus', 'btaurus', 'oaries', 'sscrofa', 'cfamiliaris', 'fcatus', 'ecaballus', 'ocuniculus', 'mdomestica', 'ggallus', 'mgallopavo', 'acarolinensis', 'tguttata', 'loculatus');} # order for 2R fishes
	if ($wgdtype eq '_3R'){@organisms = ('tnigroviridis', 'gaculeatus', 'olatipes', 'drerio');} # order for 3R fishes
}
else {@organisms = ('hsapiens', 'mmusculus', 'rnorvegicus', 'ptroglodytes', 'ggorilla', 'pabelii', 'panubis', 'csabaeus', 'mmulatta', 'cjacchus', 'btaurus', 'oaries', 'sscrofa', 'cfamiliaris', 'fcatus', 'ecaballus', 'ocuniculus', 'mdomestica', 'ggallus', 'mgallopavo', 'acarolinensis', 'tguttata', 'loculatus', 'tnigroviridis', 'gaculeatus', 'olatipes', 'drerio');} # order for 2R vertebrates
#else {@organisms = ('hsapiens', 'mmusculus', 'rnorvegicus', 'ptroglodytes', 'ggorilla');} # order for 2R vertebrates
...
foreach my $org2add (@organisms){
	...
	if ($org2add ne $organism){ # If it's not the same organism
		...
		my $otherVfile; # file for the other organism
		if ($org2add ~~ @fish){
			$otherVfile = "$org2add"."Ohno_Self+Outgp$wgdtype\_withDetails.txt";
		}
		else {$otherVfile = "$org2add"."Ohno_Self+Outgp_withDetails.txt";}
			...
			if ((exists $Orthologs{$line[0]}) && (exists $Orthologs{$line[1]})){
				...
				if (scalar @orthPairs <= 1){ # If there is just one line, or no line -> print it
					...
					if (scalar @orthPairs == 0){
						...
						print OUT "$otLine[2],$otLine[3]\t$otLine[7]\t$otLine[13]"; # 2,3 gene symbols. 7: p-outgroup. 13: p-self.
					}
				}
					...
					my @sortedVline = @$ref;
					map {$_=~s/\n//} @sortedVline;
					print OUT "$sortedVline[2],$sortedVline[3]\t$sortedVline[7]\t$sortedVline[13]";
				}
			}
			else {print OUT "\t\t";}
			print OUT "\n";
...
sub SortProbabilities {
	...
	my @sorted = sort {$a->[7] cmp $b->[7] || $a->[13] cmp $b->[13] || $b->[6] <=> $a->[6]} @temp; # sort based on 7:P-og then 13:P-self then 6:Og sup
